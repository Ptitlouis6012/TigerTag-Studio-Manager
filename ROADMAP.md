...
- ✅ **Anycubic live integration** — **LAN**: MQTTS port 9883 (TLS 1.2), broker credentials imported from AnycubicSlicerNext config; ACE multi-box/slot grid (incl. external box), control panel (jog / home / disable-motors / light / fan / nozzle+bed temp / speed-mode), filament edit sheet, HTTP-FLV camera via ffmpeg (Kobra 3 V2 `/flv`; **Kobra X** tokenized `/live/<token>` learned from MQTT `info/report` — PR #2 by @ennisj). **Cloud**: cross-platform web-login (token + email), live status / temps / ACE slots / job thumbnail, realtime device controls (fan / temp / speed) via cloud-MQTT publish + light & pause/resume/stop via signed REST `sendOrder`, printer error alerts (e.g. code 10901), and **cloud camera** — Agora/声网 WebRTC (side panel, cam wall, detached window, with RTC-token renewal), reverse-engineered from the slicer via cloud order 1001. *The earlier "cloud camera is gated server-side / out of scope" assessment was wrong.* (v1.9.0 → v1.10.8)
  ...
  - 🔜 **Gap vs Creality — remote file management**: Anycubic only *displays* the active print filename; it has no browse-stored-gcode → start-print → delete flow (Creality + Snapmaker do). Not a hardware limit — the printer stores gcode and the slicer starts prints remotely. Needs the reverse-engineered file-list / print-start / delete orders for **both** transports (LAN MQTT + cloud REST `sendOrder`). This is the only non-hardware Anycubic-vs-Creality parity gap; fits the generic plan in *🥉 Printer control panel* below.
...
- **Image CDN — is the bill egress-bound or invocation-bound?** ([#25](https://github.com/TigerTag-Project/TigerTag-Studio-Manager/issues/25)) · **S** · Studio now asks `/img` for the rendition that fits each slot instead of taking the `large` default (catalogue table: 33 kB → 3.4 kB per row, ~10x less egress). The untested trade is that several rungs of the same product mean several edge-cache keys, so more 302s on first miss. Which side the bill actually sits on would decide whether to push further (a smaller rung for rack tiles, AVIF) or to collapse rungs so every list shares one key — but the bill was checked in Sept 2026 and the whole project runs at **~8 €/month, falling**, so neither side matters yet. Parked. Two things measured while looking: the catalogue IS cached, by Chromium rather than by us — a cold reload served 25 of 25 images from the HTTP disk cache, `immutable` for a year, the 302 not even replayed — so the exposure is browser EVICTION across 13 408 products, not repeated downloads; and the proxy serves **AVIF** (25/25), so the byte figures quoted anywhere for the rungs are JPEG sizes and the real transfer is smaller. `&stream=1` and pre-caching a smaller rendition were both considered and REJECTED on cost — the reasoning is recorded in the code so it is not re-discovered as a good idea.

- **Rename the GitHub repo `TigerTag-Studio-Manager` → `Tiger-Studio-Manager`** — it is the last place still carrying "TigerTag": the product is already `Tiger Studio Manager` and so are the binaries (`Tiger-Studio-Manager-Setup-<version>.exe`). **Effort**: S. **Risk**: low, with one trap.
  - **Auto-update survives it.** `build.publish` (`owner` + `repo`) is baked into each build's `app-update.yml`, so every already-installed app keeps asking for the OLD name forever. It works because GitHub keeps a permanent redirect — repo *and* release-asset URLs — and electron-updater follows redirects. There is no `setFeedURL` anywhere in `main.js`, so nothing else to reroute. The `appId` (`io.tigertag.inventory`) and `productName` are independent of the repo, so installs upgrade in place: no duplicate app, no lost accounts or inventory.
  - **The trap**: that redirect dies the day anything re-occupies the name `TigerTag-Studio-Manager` in the org — and every pre-rename install then stops updating, *silently*. After the rename that name is burnt; never reuse it.
  - **Verify, don't assume.** A broken updater produces no complaints, just users frozen on an old version. Keep a pre-rename build installed, rename, publish the next version, and confirm the old build actually offers it.
  - **Order matters**: rename on GitHub FIRST, then ship the code changes — pointing `package.json` at a repo that does not exist yet would break the *next* build's publish.
  - **What to change** (5 code references + docs): `package.json` `build.publish.repo` (the only one that affects the updater), `main.js` (app "website" link), `renderer/inventory.js` ×2 (sidebar GitHub button + sign-in screen), `scripts/extract-changelog.mjs` ×2 (the "Full changelog" footer of every release note), then `README.md`, `TRADEMARK.md`, `llms.txt`. The CI needs nothing — the workflows use `github.repository`. Outside this repo: `CLAUDE.local.md`, and the repo links in TigerSystem-Docs, TigerTag_Hub and TigerTag_Firebase_Integration.
